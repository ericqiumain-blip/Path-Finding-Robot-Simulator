...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main>
        <section className="page-heading"><div><div className="eyebrow">AUTONOMOUS OPERATIONS</div><h1>{view === 'live' ? 'Warehouse overview' : view === 'fleet' ? 'Robot fleet' : 'Performance analytics'}<span className="heading-dot">.</span></h1><p>{view === 'live' ? 'Every robot. Every route. One coordinated floor.' : view === 'fleet' ? 'Inspect every robot and the work it is doing.' : 'Measure the decisions behind every delivery.'}</p></div><div className="export-group"><a className="secondary-button" href="/api/metrics?format=csv" download><Icon name="download" size={16}/>Export CSV</a><a className="icon-button" href="/api/metrics?format=json" download aria-label="Export metrics as JSON" title="Export JSON"><Icon name="box" size={17}/></a></div></section>
